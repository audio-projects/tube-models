...
function makeAdc(override: Record<string, number | number[]> = {}): AdcData {
    ...
    const value: AdcData = {
        ...
        plateCurrentRawValue: 0,
        plateCurrentRaw: 0,
        screenCurrentBytes: [0, 0],
        screenCurrentValue: 0,
        screenCurrent: 0,
        screenCurrentRawBytes: [0, 0],
        screenCurrentRawValue: 0,
        screenCurrentRaw: 0,
        plateVoltageBytes: [0, 0],
        plateVoltageValue: 0,
        plateVoltage: 0,
        screenVoltageBytes: [0, 0],
        screenVoltageValue: 0,
        screenVoltage: 0,
        positivePowerSupplyVoltageBytes: [0, 0],
        positivePowerSupplyVoltageValue: 0,
        positivePowerSupplyVoltage: 0,
        negativePowerSupplyVoltageBytes: [0, 0],
        negativePowerSupplyVoltageValue: 0,
        negativePowerSupplyVoltage: 0,
        plateCurrentGainValue: 0,
        plateCurrentGain: 0,
        screenCurrentGainValue: 0,
        screenCurrentGain: 0,
        plateCurrentAverage: 0,
        screenCurrentAverage: 0
    };
...
describe('UTracerService', () => {
    ...
    describe('UTracerService.readPowerSupplyVoltage', () => {

        it('returns 0 when ADC value is 0', () => {
            // arrange
            const adc = makeAdc({positivePowerSupplyVoltage: 0});
            // act
        ...
        it('returns expected max voltage for ADC=1023 using default calibration', () => {
            // arrange
            const adc = makeAdc({positivePowerSupplyVoltage: 1023});
            // act
            const result = service.readPowerSupplyVoltage(adc);
            // assert
        ...
        it('returns voltage for ADC=872 using default calibration', () => {
            // arrange
            const adc = makeAdc({positivePowerSupplyVoltage: 872});
            // act
            const result = service.readPowerSupplyVoltage(adc);
            // assert
    ...
    describe('UTracerService.readPlateVoltage', () => {
        ...
        it('computes plate voltage when plate and power supply ADC are zero', () => {
            // arrange
            const adc = makeAdc({ plateVoltageValue: 0, positivePowerSupplyVoltage: 0 });
        ...
        it('computes plate voltage for max plate ADC with zero power supply ADC - V3', () => {
            // arrange
            const adc = makeAdc({ plateVoltageValue: 1023, positivePowerSupplyVoltage: 0 });
        ...
        it('computes plate voltage for plate ADC=872 and power supply ADC=512', () => {
            // arrange
            const adc = makeAdc({ plateVoltageValue: 872, positivePowerSupplyVoltage: 20.0 });
    ...
    describe('UTracerService.readScreenVoltage', () => {
        ...
        it('computes screen voltage when screen and power supply ADC are zero', () => {
            // arrange
            const adc = makeAdc({ screenVoltageValue: 0, positivePowerSupplyVoltage: 0 });
            const averaging: Averaging = 0;
            // act
        ...
        it('computes screen voltage for max screen ADC with zero power supply ADC - V3', () => {
            // arrange
            const adc = makeAdc({ screenVoltageValue: 1023, positivePowerSupplyVoltage: 0 });
        ...
        it('computes screen voltage for screen ADC=872 and power supply ADC=512', () => {
            // arrange
            const adc = makeAdc({ screenVoltageValue: 872, positivePowerSupplyVoltage: 20.0 });
            const averaging: Averaging = 0;
            // act
            const result = service.readScreenVoltage(adc, averaging);
            // assert
    ...
    describe('UTracerService.calculatePlateBytes', () => {
        ...
        it('computes ADC values for zero plate and power supply voltages', () => {
            // arrange
            const positivePowerSupplyVoltage = 0;
            const plateVoltage = 0.20;
            // act
            const result = service.calculatePlateBytes(positivePowerSupplyVoltage, plateVoltage);
            // assert
        ...
        it('computes ADC values for max plate voltage and power supply voltage=20.0 - V3', () => {
            // arrange
            const positivePowerSupplyVoltage = 20.0;
            const plateVoltage = 350.79;
            // act
            const result = service.calculatePlateBytes(positivePowerSupplyVoltage, plateVoltage);
            // assert
    ...
    describe('UTracerService.calculateScreenBytes', () => {
        ...
        it('computes ADC values for zero screen and power supply voltages', () => {
            ...
            // act
            const result = service.calculateScreenBytes(positivePowerSupplyVoltage, screenVoltage);
            // assert
            expect(result).toEqual([0, 0]);
        });
    ...
    describe('UTracerService.calculateHeaterBytes', () => {
        ...
        it('returns [0, 0] when power supply voltage is zero (division by zero protection)', () => {
            ...
            // act
            const result = service.calculateHeaterBytes(positivePowerSupplyVoltage, heaterVoltage);
            // assert
            expect(result).toEqual([0, 0]);
        });
        ...
        it('computes PWM value for zero heater voltage', () => {
            // arrange
            const positivePowerSupplyVoltage = 24.0;
        ...
        it('respects PWM scaling formula: n = 1023 * (voltage/powerSupply)^2', () => {
            // arrange
            const positivePowerSupplyVoltage = 25.2;
            const heaterVoltage = 6.3;
            // act
            const result = service.calculateHeaterBytes(positivePowerSupplyVoltage, heaterVoltage);
            // assert
    ...
    describe('UTracerService.calculateGridBytes', () => {
        ...
        it('computes bytes for zero grid voltage (boundary case)', () => {
            // arrange
            const negativePowerSupplyVoltage = -50;
            const gridVoltage = 0;
            // act
            const result = service.calculateGridBytes(negativePowerSupplyVoltage, gridVoltage);
            // assert
            expect(result).toEqual([0, 0]);
        });
        ...
        it('respects varying negativePowerSupplyVoltage parameter', () => {
            // arrange
            const negativePowerSupplyVoltage = -40;
            const gridVoltage = -0.5;
            spyOnProperty(service, 'grid1VoltVoltageGain', 'get').and.returnValue(1.0);
            // act
            const result = service.calculateGridBytes(negativePowerSupplyVoltage, gridVoltage);
