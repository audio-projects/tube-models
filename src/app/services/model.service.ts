...
export class ModelService {
    ...
    getDerkEModelDefinition(secondaryEmissions: boolean): string {
        ...
        if (secondaryEmissions) {
            ...
            return [
                ...
                'E2 8 0 VALUE={S*V(1,3)*(1+(EXP(-2*ALPHAP*(V(1,3)-(V(4,3)/LAMBDA-V*V(2,3)-W)))-1)/(EXP(-2*ALPHAP*(V(1,3)-(V(4,3)/LAMBDA-V*V(2,3)-W)))+1))}',
                'R2 8 0 1G',
                'G1 1 3 VALUE={(0.5*PWR(V(7),EX)*(1+SGN(V(7))))*(1/KG1-1/KG2+A*V(1,3)/KG1-V(8)/KG2-EXP(-PWR(BETA*V(1,3),1.5))*(ALPHA/KG1+ALPHAS/KG2))}',
                'R3 1 3 1G',
    ...
    getTriodeModel(tube?: TubeInformation): {name: string, model: string} {
        // parameters
    ...
    getPentodeModel(tube?: TubeInformation): {name: string, model: string} {
        // parameters
    ...
    getDerkModel(tube?: TubeInformation): {name: string, model: string} {
        // parameters
    ...
    getDerkEModel(tube?: TubeInformation): {name: string, model: string} {
        // parameters
