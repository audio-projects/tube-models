...
export const derkModelError = function (files: File[], kp: number, mu: number, kvb: number, ex: number, kg1: number, kg2: number, a: number, alphaS: number, beta: number, secondaryEmission: boolean, s: number, alphaP: number, lambda: number, v: number, w: number, maximumPlateDissipation: number): {sse: number, rmse: number} {
    // result
    let error = 0;
    let count = 0;
    // loop data files
