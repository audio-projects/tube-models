...
// derkEModelError
export const derkEModelError = function (files: File[], kp: number, mu: number, kvb: number, ex: number, kg1: number, kg2: number, a: number, alpha: number, alphaS: number, beta: number, secondaryEmission: boolean, s: number, alphaP: number, lambda: number, v: number, w: number, maximumPlateDissipation: number) {
    // result
    let r = 0;
    // loop data files
            ...
            for (const series of file.series) {
                ...
                for (const point of series.points) {
                    ...
                    if (point.ep * point.ip * 1e-3 <= maximumPlateDissipation) {
                        // calculate currents
                        const c = derkEModel(point.ep, point.eg + file.egOffset, point.es || 0, kp, mu, kvb, ex, kg1, kg2, a, alpha, alphaS, beta, secondaryEmission, s, alphaP, lambda, v, w);
                        // residuals
