...
export const normanKorenNewPentodeModelError = function (files: File[], mu: number, kp: number, kvb: number, ex: number, kg1: number, kg2: number, egOffset: number, maximumPlateDissipation: number) {
    // result
    let r = 0;
    // loop data files
            ...
            for (const series of file.series) {
                ...
                for (const point of series.points) {
                    ...
                    if (point.ep * point.ip * 1e-3 <= maximumPlateDissipation) {
                        // calculate currents
                        const c = normanKorenNewPentodeModel(point.ep, point.eg + egOffset, point.es ?? 0, kp, mu, kvb, ex, kg1, kg2);
                        // residuals
