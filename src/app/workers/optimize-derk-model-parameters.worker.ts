...
const optimizeWithLevenbergMarquardt = function (files: File[], maximumPlateDissipation: number, mu: number, ex: number, kg1: number, kp: number, kvb: number, kg2: number, a: number, alphaS: number, beta: number, secondaryEmission: boolean, s: number, alphaP: number, lambda: number, v: number, w: number, trace?: Trace) {
            ...
            for (const series of file.series) {
                ...
                for (const point of series.points) {
                    ...
                    if ((point.ip + (point.is ?? 0)) > 0 && point.ep * (point.ip + (point.is ?? 0)) * 1e-3 <= maximumPlateDissipation) {
                        // calculate currents
                        const currents = derkModel(point.ep, point.eg + file.egOffset, point.es ?? 0, kp * x3, mu * x0, kvb * x4, ex * x1, kg1 * x2, kg2 * x5, a * x6, alphaS * x7, beta * x8, secondaryEmission, s * x9, alphaP * x10, lambda * x11, v * x12, w * x13);
                        // residuals
    ...
    self.postMessage({
        type: 'log',
        text: `Optimizing Derk Model parameters using the Levenberg-Marquardt algorithm, Objective function value: ${(derkModelError(files, kp, mu, kvb, ex, kg1, kg2, a, alphaS, beta, secondaryEmission, s, alphaP, lambda, v, w, maximumPlateDissipation) * 1e-6).toExponential()}`
    });
    // optimize
...
const optimizeWithPowell = function (files: File[], maximumPlateDissipation: number, mu: number, ex: number, kg1: number, kp: number, kvb: number, kg2: number, a: number, alphaS: number, beta: number, secondaryEmission: boolean, s: number, alphaP: number, lambda: number, v: number, w: number, trace?: Trace) {
    ...
    postMessage({
        type: 'log',
        text: `Optimizing Derk Model parameters using the Powell algorithm, Objective function value: ${(derkModelError(files, kp, mu, kvb, ex, kg1, kg2, a, alphaS, beta, secondaryEmission, s, alphaP, lambda, v, w, maximumPlateDissipation) * 1e-6).toExponential()}`,
    });
    ...
    const leastSquares1 = function (x: number[]): number {
        ...
        return derkModelError(files, kp, mu, kvb, ex, kg1, kg2, a, alphaS, beta, secondaryEmission, s, alphaP, lambda, v, w, maximumPlateDissipation);
    };
    ...
    if (result.converged) {
        ...
        postMessage({
            type: 'log',
            text: `1. Derk Model parameters: mu=${mu}, ex=${ex}, kg1=${kg1}, kp=${kp}, kvb=${kvb}, kg2=${kg2}, a=${a}, alphaS=${alphaS}, beta=${beta}, s=${s}, alphaP=${alphaP}, lambda=${lambda}, v=${v}, w=${w}, Objective function value: ${(derkModelError(files, kp, mu, kvb, ex, kg1, kg2, a, alphaS, beta, secondaryEmission, s, alphaP, lambda, v, w, maximumPlateDissipation) * 1e-6).toExponential()}, iterations: ${result.iterations}`,
        });
        ...
        if (result.converged) {
            ...
            postMessage({
                type: 'log',
                text: `2. Derk Model parameters: mu=${parameters.mu}, ex=${parameters.ex}, kg1=${parameters.kg1}, kp=${parameters.kp}, kvb=${parameters.kvb}, kg2=${parameters.kg2}, a=${parameters.a}, alphaS=${parameters.alphaS}, beta=${parameters.beta}, s=${parameters.s}, alphaP=${parameters.alphaP}, lambda=${parameters.lambda}, v=${parameters.v}, w=${parameters.w}, Objective function value: ${(derkModelError(files, parameters.kp, parameters.mu, parameters.kvb, parameters.ex, parameters.kg1, parameters.kg2, parameters.a, parameters.alphaS, parameters.beta, secondaryEmission, parameters.s, parameters.alphaP, parameters.lambda, parameters.v, parameters.w, maximumPlateDissipation) * 1e-6).toExponential()}, iterations: ${result.iterations}`,
            });
            // return model parameters
            return parameters;
        }
...
addEventListener('message', ({ data }) => {
    ...
    // estimate parameters
    const estimates = estimateDerkParameters({}, files, maximumPlateDissipation, secondaryEmission, trace);
    // update parameters
