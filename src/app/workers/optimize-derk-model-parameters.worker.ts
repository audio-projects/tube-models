...
const optimizeWithLevenbergMarquardt = function (files: File[], mu: number, ex: number, kg1: number, kp: number, kvb: number, kg2: number, a: number, alphaS: number, beta: number, secondaryEmission: boolean, s: number, alphaP: number, lambda: number, v: number, w: number, trace?: Trace) {
    ...
    self.postMessage({
        type: 'log',
        text: `Optimizing Derk Model parameters using the Levenberg-Marquardt algorithm, Root Mean Square Error: ${derkModelError(files, kp, mu, kvb, ex, kg1, kg2, a, alphaS, beta, secondaryEmission, s, alphaP, lambda, v, w).rmse.toExponential()}`
    });
    // optimize
...
const optimizeWithPowell = function (files: File[], mu: number, ex: number, kg1: number, kp: number, kvb: number, kg2: number, a: number, alphaS: number, beta: number, secondaryEmission: boolean, s: number, alphaP: number, lambda: number, v: number, w: number, trace?: Trace) {
    ...
    postMessage({
        type: 'log',
        text: `Optimizing Derk Model parameters using the Powell algorithm, Root Mean Square Error: ${derkModelError(files, kp, mu, kvb, ex, kg1, kg2, a, alphaS, beta, secondaryEmission, s, alphaP, lambda, v, w).rmse.toExponential()}`,
    });
    ...
    let sumOfSquaredErrors = function (x: number[]): number {
        ...
        return derkModelError(files, kp, mu, kvb, ex, kg1, kg2, a, alphaS, beta, secondaryEmission, s, alphaP, lambda, v, w).sse;
    };
    ...
    if (result.converged) {
        ...
        postMessage({
            type: 'log',
            text: `1. Derk Model parameters: mu=${mu}, ex=${ex}, kg1=${kg1}, kp=${kp}, kvb=${kvb}, kg2=${kg2}, a=${a}, alphaS=${alphaS}, beta=${beta}, s=${s}, alphaP=${alphaP}, lambda=${lambda}, v=${v}, w=${w}, Root Mean Square Error: ${derkModelError(files, kp, mu, kvb, ex, kg1, kg2, a, alphaS, beta, secondaryEmission, s, alphaP, lambda, v, w).rmse.toExponential()}, iterations: ${result.iterations}`,
        });
...
addEventListener('message', ({ data }) => {
    ...
    // estimate parameters
    const estimates = estimateDerkParameters({}, files, secondaryEmission, trace);
    // update parameters
